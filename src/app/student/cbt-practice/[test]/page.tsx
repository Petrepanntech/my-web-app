...
const TEST_DURATION_PER_QUESTION_SECONDS = 90; // 1.5 minutes per question
...
export default function CbtTestPage() {
  ...
  useEffect(() => {
    ...
    if (subjects.length === 0) {
        toast({ title: "Error", description: "No subjects selected.", variant: "destructive" });
    ...
    if (filteredQuestions.length === 0) {
        toast({ title: "Error", description: "No questions found for the selected subjects.", variant: "destructive" });
        router.push("/student/cbt-practice");
        return;
    }

    setQuestions(filteredQuestions);
    setTimeLeft(filteredQuestions.length * TEST_DURATION_PER_QUESTION_SECONDS);
    setIsLoading(false);
  ...
  if (isLoading) {
    return (
      <DashboardAuthWrapper requiredRole="student">
        <div className="container mx-auto max-w-4xl py-12">Loading test...</div>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <div className="mb-8 min-h-[6rem]">
              <p className="font-semibold text-lg mb-2">Question {currentQuestionIndex + 1}/{questions.length} ({currentQuestion.subject})</p>
